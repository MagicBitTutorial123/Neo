...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Check if user is new based on mission progress or if they're a first-time OAuth user
    if (userData?.isNewUser || (userData?.missionProgress !== undefined && userData?.missionProgress < 2)) {
      setIsNewUser(true);
    }
    
    // Check if this is a first-time OAuth user (has profile but no mission progress)
    const checkFirstTimeOAuth = async () => {
      try {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
          // Check if user has a profile but no mission progress
          ...
          if (profile && (!userData?.missionProgress || userData.missionProgress === 0)) {
            setIsFirstTimeOAuth(true);
            setShowProfileUpdateNotification(true);
            console.log('🆕 First-time OAuth user detected');
          }
        }
      } catch (error) {
  ...
  // Show onboarding for new users from signup OR first-time OAuth users
  const shouldShowOnboarding = (isNewUser && isFromSignup) || isFirstTimeOAuth;

  console.log("🔍 User data:", userData);
  console.log("🔍 Is new user:", isNewUser);
  console.log("🔍 Is first-time OAuth:", isFirstTimeOAuth);
      ...
      <div className="flex min-h-screen bg-[#F8F9FC] overflow-x-hidden">
        ...
        >
          ...
          {shouldShowOnboarding ? (
            // New User Onboarding (Robot Animation)
